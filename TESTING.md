...
- [ ] **Pending:** Send a normal prompt, queued follow-up, and promoted steer; confirm every path snapshots all active-note attached/detached annotations exactly once as bounded structured context, immediately clears their highlights/list, and exits pick mode while visible chat text remains unchanged. Attached ranges/blocks should remain visibly masked by gray bars with a left-to-right accent sweep while processing. Retrieve a queued follow-up and confirm its annotations return to the note and its mask clears.
- [ ] **Pending:** Confirm exact text selections mask only the selected characters and multi-line/block masks follow the text width and height without changing wrapping, spacing, or line layout. Add a deliberately misspelled selection and confirm its red spellcheck underline is hidden while masked.
