...
- [ ] **Pending:** Compare source mode, live preview, and reading mode: text selections and paragraph/block annotations must all use the same subtle accent background and underline, with no left vertical bar. In pick mode, hovered/focused paragraphs must use a rounded accent outline rather than a vertical bar.
