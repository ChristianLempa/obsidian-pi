...
- Chat history is stored as one versioned JSON file per chat in the configurable vault-relative `chats` folder. Pi runtime sessions remain separate under the plugin's local `pi-sessions` directory.
